...
from pyspark.sql.functions import col, expr, udf, ceil, percent_rank,from_json, explode,ntile, desc, asc, round, when, lit, pow, current_timestamp, log, monotonically_increasing_id, row_number, concat, array, struct, rank, dense_rank,  from_json, to_json, collect_list, transform,flatten
...
backside_missing_values_cleaning = {'BACKSIDE':'Blank'}
initial_data = initial_data.na.fill(backside_missing_values_cleaning)

gender_values_cleaning = {'Ladies\'':'WOMEN'
                          , 'MEN/UNISEX':'MEN OR WOMEN'
                          ,'MEN/UNISEX,MEN':'MEN OR WOMEN'
                          ,'Men\'s': 'MEN'
                          ,'UNISEX': 'MEN OR WOMEN'}
initial_data = initial_data.na.replace(gender_values_cleaning)
...
items_df = items_df.withColumn("INDEX", f.sha2(f.concat(*(f.col(c).cast("string") for c in ["PRODUCT_KEY", "MARKET","USER_SELECTIONS_CONFIG_ONLY"])), 256))
items_pd = items_df.toPandas()
items_df = items_df.withColumn("USER_SELECTIONS_CONFIG_ONLY_STR", to_json(col("USER_SELECTIONS_CONFIG_ONLY"))).cache()
...
configs_df = configs_df.drop(*["USER_SELECTIONS","USER_SELECTIONS_CONFIG_ONLY_STR",'SIZE']).cache()
...
configs_df.where((configs_df.PRODUCT_KEY == 'PRD-BCLHG9OCQ') ).display() #& (configs_df.COLOUR == '#ff0000')

# COMMAND ----------

configs_df.where((configs_df.PRODUCT_GROUP == 'T-shirts') & (configs_df.GENDER == '#FFFFFF')).display()
...
text_embedding_df = prepare_embedding(text_df, spark, col_to_embed='COMBINED_TEXT', dimensions=3
                                             )
...
gender_df = configs_df.withColumn("GENDER_TEXT", #configs_df["GENDER"], lit(" "), configs_df["PRODUCT_GROUP"],
                    # Tier 1 (8)
                    concat(lit("The "), configs_df["PRODUCT_GROUP"],  lit(" is specifically intended for "), configs_df["GENDER"], lit(". "))                   
                   )

# COMMAND ----------

gender_embedding_df = prepare_embedding(gender_df, spark, col_to_embed='GENDER_TEXT', dimensions=2
                                             )
gender_embedding_df = gender_embedding_df.withColumnRenamed('embeddings', 'GENDER_EMBEDDINGS')
gender_embedding_df.display()

# COMMAND ----------

indexes_test = ['5f357d44bff9f91f883704952d7b57594f25c16d306bcbaa18408d42876542b3','46f741afcdc9812e7e6f7eae94f68ccb59c8092ab02505be97858c1c3c9016d7','2ed391a2115abfce869319ab91048bddd2c23c0b6550b8b61d9590d076b0c670','439aede2009c0af80b117741bb453e7d08f521f18114613b013e4488010a647b']
texte_test = text_embedding_df.where(text_embedding_df.INDEX.isin(indexes_test)) #.cache()
texte_test.display()

# COMMAND ----------

gendere_test = gender_embedding_df.where(gender_embedding_df.INDEX.isin(indexes_test)) #.cache()
gendere_test.display()
...
num_concatenated_inputs = Concatenate()([input_price])
num_dense_layer = Dense(1, activation='relu')(num_concatenated_inputs)
num_output_layer = Dense(1, activation='linear')(num_dense_layer)

num_model = Model(inputs=[input_price], outputs= num_output_layer)
...
num_embeddings = num_encoder_model.predict([configs_pd['MOQ_UNIT_PRICE_SCALED']])
...
num_color_concatenated_inputs = Concatenate()([input_l_colour, input_a_colour, input_b_colour])
num_color_dense_layer = Dense(3, activation='relu')(num_color_concatenated_inputs)
num_color_output_layer = Dense(3, activation='linear')(num_color_dense_layer)

num_color_model = Model(inputs=[input_l_colour, input_a_colour, input_b_colour], outputs= num_color_output_layer)

# Compile the model
num_color_model.compile(optimizer='adam', loss='mse')

# Extract embeddings for your data
num_color_encoder_model = Model(inputs=num_color_model.input, outputs=num_color_model.layers[-2].output)

num_color_embeddings = num_color_encoder_model.predict([configs_pd['L_COLOUR_SCALED'], configs_pd['A_COLOUR_SCALED'], configs_pd['B_COLOUR_SCALED']])

# Add embeddings to the DataFrame
configs_pd['NUMERICAL_COLOR_EMBEDDINGS'] = list(num_color_embeddings)
configs_pd['NUMERICAL_COLOR_EMBEDDINGS'] = configs_pd['NUMERICAL_COLOR_EMBEDDINGS'].apply(lambda x: x.tolist())
configs_pd['NUMERICAL_COLOR_EMBEDDINGS'] = configs_pd['NUMERICAL_COLOR_EMBEDDINGS'].apply(json.dumps)
...
numerical_embedding_df = numerical_embedding_df.select("INDEX", "NUMERICAL_EMBEDDINGS", "NUMERICAL_COLOR_EMBEDDINGS", "CATEGORICAL_EMBEDDINGS")
...
numerical_embedding_df = numerical_embedding_df.withColumn("NUMERICAL_COLOR_EMBEDDINGS", from_json(col("NUMERICAL_COLOR_EMBEDDINGS"), array_schema))
...
nume_test = numerical_embedding_df.where(numerical_embedding_df.INDEX.isin(indexes_test)) #.cache()
nume_test.display()
...
pre_test = full_df.where(full_df.INDEX.isin(indexes_test))
pre_test.display()

# COMMAND ----------

full_df = full_df.withColumn('FULL_EMBEDDING', flatten(array(col("TEXT_EMBEDDINGS"), col("NUMERICAL_EMBEDDINGS"), col("NUMERICAL_COLOR_EMBEDDINGS"), col("CATEGORICAL_EMBEDDINGS"), col('GENDER_EMBEDDINGS'))))

# COMMAND ----------

test = full_df.where(full_df.INDEX.isin(indexes_test))
test.display()
...
pairwise_df.where((pairwise_df.SOURCE_INDEX == indexes_test[2]) & (pairwise_df.COMPARISON_INDEX == indexes_test[3])).display()
...
configs_df.select('MIN_QUANTITY').distinct().collect()
...
# restrict one product config per product: most similar product config
comparison_prod_ranking = Window.partitionBy("SOURCE_INDEX","COMPARISON_PRODUCT_KEY").orderBy(desc("PRODUCT_SIMILARITY"), "COMPARISON_INDEX")
...
s_fields = write_out_example_final.schema["COMPARISON_REFERENCE_KEY_NO_SIZE"].dataType.names # ['fld']

write_out_example_final = write_out_example_final.withColumn(
    'COMPARISON_REFERENCE_KEY_NO_SIZE', 
    f.struct(*([f.col('COMPARISON_REFERENCE_KEY_NO_SIZE')[c].alias(c) for c in s_fields] + [write_out_example_final.PRODUCT_SIMILARITY.alias('similarity_score')]))
)
...
# # write final table to delta table
# unity_schema = 'dna_e2_dev_productrecommendations_a0ce4e86_2360_460e_8100_81a39afe3cf0'
# grouped_demo_example \
#     .write \
#     .mode("overwrite") \
#     .option("overwriteSchema", "true") \
#     .saveAsTable(f"vista.`{unity_schema}`.ps_demo_items")
