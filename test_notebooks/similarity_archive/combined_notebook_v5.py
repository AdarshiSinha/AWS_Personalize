...
from pyspark.sql.functions import col, expr, udf, ceil, percent_rank,from_json, explode,ntile, desc, asc, round, when, lit, pow, current_timestamp, log, monotonically_increasing_id, row_number, concat, array, struct, rank, dense_rank,  from_json, to_json
...
configs_df = items_df.drop(*[items_df.SIZE, items_df.USER_SELECTIONS, items_df.USER_SELECTIONS_CONFIG_ONLY]).dropDuplicates()
...
configs_df.where( (configs_df.SLEEVE_STYLE =='LONG SLEEVE') ).display()
configs_df.count()
...
full_df = (text_embedding_df#.join(tier_2_text_embedding_df, ["INDEX"])
...
full_df.where(full_df.INDEX == 'a4846131a3d9443e0d61aaad6e776df647829c97f6beff7fd80f525d046f3f87').display()
...
full_df.where(col("NUMERICAL_EMBEDDINGS").isNull()).display()
...
pairwise_df = spark.createDataFrame(
    ...
    ["SOURCE_INDEX", "COMPARISON_INDEX", "PRODUCT_SIMILARITY"]
)
...
pairwise_df.where((pairwise_df.SOURCE_INDEX == '9149f18bdc5cdebfeb2139e7b1456507d8126f3bd67750f306b55d2458475436') & (pairwise_df.COMPARISON_INDEX == 'a4846131a3d9443e0d61aaad6e776df647829c97f6beff7fd80f525d046f3f87')).display()
...
source_df = configs_df.join(pairwise_df, configs_df.INDEX == pairwise_df.SOURCE_INDEX).select('SOURCE_INDEX','PRODUCT_KEY', 'COMPARISON_INDEX', 'PRODUCT_SIMILARITY','COLOUR_SIMILARITY').withColumnRenamed("PRODUCT_KEY", "SOURCE_PRODUCT_KEY")
...
write_out_example.where(ranked_final_df.SOURCE_INDEX == '70960b2df95b46e159790821c284da96e4e06ebc65d36c055116756df82b6d25').display()
