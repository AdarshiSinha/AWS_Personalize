...
def prepare_embedding(text_df,spark, col_to_embed,culture = "en"):
        '''
...
product_text_cols = ['PRODUCT_NAME','SUBCATEGORY', 'PRODUCT_GROUP','GENDER','SLEEVE_STYLE']
...
product_text_df = configs_df.withColumn("COMBINED_PRODUCT_TEXT",
                    concat(
                        lit("The PRODUCT NAME is "), configs_df["PRODUCT_NAME"], lit(". "),
...
product_text_embedding_df = prepare_embedding(product_text_df, spark, col_to_embed='COMBINED_PRODUCT_TEXT')
product_text_embedding_df.display()
...
features_text_cols = ['BRAND_NAME','MATERIAL', 'COLLAR_TYPE','DECORATION_LOCATION_1','DECORATION_TECHNOLOGY_1']


#logic for concat
for c in features_text_cols:
    print(f"lit(\"The {c.replace('_', ' ')} is \"), configs_df[\"{c}\"], lit(\". \"),")

# COMMAND ----------

features_text_df = configs_df.withColumn("COMBINED_FEATURES_TEXT",
                    concat(
...
features_text_embedding_df = prepare_embedding(features_text_df, spark, col_to_embed='COMBINED_FEATURES_TEXT')
features_text_embedding_df = features_text_embedding_df.select(features_text_embedding_df.INDEX, features_text_embedding_df.embeddings.alias("FEATURES_EMBEDDINGS"))
features_text_embedding_df.display()
...
config_text_cols = ['HUMAN_COLOR']

#logic for concat
for c in config_text_cols:
    print(f"lit(\"The {c.replace('_', ' ')} is \"), configs_df[\"{c}\"], lit(\". \"),")

# COMMAND ----------

config_text_df = configs_df.withColumn("COMBINED_CONFIG_TEXT",
                    concat(
                        lit("The HUMAN COLOR is "), configs_df["HUMAN_COLOR"], lit(". ")
                   ))

# COMMAND ----------

config_text_embedding_df = prepare_embedding(config_text_df, spark, col_to_embed='COMBINED_CONFIG_TEXT')
config_text_embedding_df = config_text_embedding_df.select(config_text_embedding_df.INDEX, config_text_embedding_df.embeddings.alias("CONFIG_EMBEDDINGS"))
config_text_embedding_df.display()
...
# Concatenate all scaled numerical features directly
#concatenated_inputs = Concatenate()([input_l_colour, input_a_colour, input_b_colour, input_size_numerical, input_moq])
concatenated_inputs = Concatenate()([input_l_colour, input_a_colour, input_b_colour, input_moq])
...
# Create the model
#model = Model(inputs=[input_l_colour, input_a_colour, input_b_colour, input_size_numerical, input_moq], outputs=output_layer)
model = Model(inputs=[input_l_colour, input_a_colour, input_b_colour, input_moq], outputs=output_layer)
...
embeddings = encoder_model.predict([configs_pd['L_COLOUR_SCALED'], configs_pd['A_COLOUR_SCALED'], configs_pd['B_COLOUR_SCALED'], configs_pd['MOQ_SCALED']])

# Add embeddings to the DataFrame
configs_pd['NUMERICAL_EMBEDDINGS'] = list(embeddings)

# COMMAND ----------

numerical_embedding_df = spark.createDataFrame(configs_pd)
numerical_embedding_df = numerical_embedding_df.select("INDEX","NUMERICAL_EMBEDDINGS")
...
full_df = (product_text_embedding_df.join(features_text_embedding_df, ["INDEX"])
              .join(config_text_embedding_df, ["INDEX"])
              .join(numerical_embedding_df, ["INDEX"]))

full_df = full_df.withColumn('FULL_EMBEDDING', concat(col('embeddings'), col('FEATURES_EMBEDDINGS'), col('CONFIG_EMBEDDINGS'), col('NUMERICAL_EMBEDDINGS')))
full_df.display()
...
lookup_items = ['2e0916441288b9764ac3923ec6c4a7a161f34bfbe82f2a864d554ae2a222c87b','d4d7edaeac4d95d8c0737e97ca28d53977bddbe19e237dac9529af0f639671d0']
