...
gender_df = configs_df.withColumn("GENDER_TEXT", #configs_df["GENDER"], lit(" "), configs_df["PRODUCT_GROUP"],
                    # Tier 1 (8)
                    concat(lit("The product"),  lit(" is specifically intended for "), configs_df["GENDER"], lit(". "))                   
                   )
...
num_dense_layer = Dense(1, activation='relu')(num_concatenated_inputs) #3
num_output_layer = Dense(1, activation='linear')(num_dense_layer)

num_model = Model(inputs=[input_price #,input_l_colour, input_a_colour, input_b_colour
                          ], outputs= num_output_layer)
...
num_embeddings = num_encoder_model.predict([configs_pd['MOQ_UNIT_PRICE_SCALED']
                                            #, configs_pd['L_COLOUR_SCALED'], configs_pd['A_COLOUR_SCALED'], configs_pd['B_COLOUR_SCALED']
                                            ])
...
full_df = full_df.withColumn('FULL_EMBEDDING', flatten(array(col("TEXT_EMBEDDINGS"), col("NUMERICAL_EMBEDDINGS"), col("CATEGORICAL_EMBEDDINGS"), col('GENDER_EMBEDDINGS'))))
...
pairwise_df = pairwise_df.join(configs_df, pairwise_df.SOURCE_INDEX == configs_df.INDEX).select("SOURCE_INDEX","COMPARISON_INDEX","PRODUCT_SIMILARITY",col("COLOUR").alias("SOURCE_COLOUR"))
pairwise_df = pairwise_df.join(configs_df, pairwise_df.COMPARISON_INDEX == configs_df.INDEX).select("SOURCE_INDEX","COMPARISON_INDEX","PRODUCT_SIMILARITY","SOURCE_COLOUR",col("COLOUR").alias("COMPARISON_COLOUR"))

# COMMAND ----------

def color_similarity(source_color, comparison_color):
    import numpy as np
    def patch_asscalar(a):
        return a.item()
    setattr(np, "asscalar", patch_asscalar)
    return delta_e_cmc(hex_to_lab(source_color), hex_to_lab(comparison_color))

color_similarity_udf = udf(color_similarity, FloatType())

# COMMAND ----------

pairwise_df = pairwise_df.withColumn("COLOUR_SIMILARITY", color_similarity_udf(col("SOURCE_COLOUR"), col("COMPARISON_COLOUR")))
